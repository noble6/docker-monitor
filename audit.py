...
def scan_trivy(image_name: str, output_file: str) -> dict[str, Any]:
    ...
    stdout, enabled = load_or_run_tool("trivy", image_name, ["trivy", "image", "--format", "json", image_name], output_file)
    if not enabled:
...
def scan_dockle(image_name: str, output_file: str) -> dict[str, Any]:
    stdout, enabled = load_or_run_tool("dockle", image_name, ["dockle", "-f", "json", image_name], output_file)
    if not enabled:
...
def aggregate_scan(image_name: str, output_file: str) -> dict[str, Any]:
    ...
    agg = {
        ...
        "critical_cves": list(t_cves["CRITICAL"].union(g_cves["CRITICAL"])),
